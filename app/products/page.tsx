...
//     }}
//     onEnquiry={(p) => {
//       // open your enquiry modal or do the enquiry flow
//       // example: open existing ProductEnquiryModal if you have it
//       setIsEnquiryModalOpen(false)
//       // setSelectedProduct(p)
//       // open another modal for enquiry if you want
//       window.open(`mailto:sales@sensokart.com?subject=Enquiry for ${encodeURIComponent(p.name)}`)
//     }}
//     onWhatsApp={(p) => {
//       const message = `Hi, I'm interested in ${p.name}. Please provide more details and pricing.`
//       window.open(`https://wa.me/9494122101?text=${encodeURIComponent(message)}`, '_blank')
//     }}
//     onEmail={(p) => {
//       const subject = `Enquiry for ${p.name}`
//       const body = `Hi,\n\nI'm interested in ${p.name}.\n\nProduct Details:\n${p.description || ''}`
//       window.open(`mailto:sales@sensokart.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`)
//     }}
//   />
// )}
//     </div>
//   )
// }
// app/(whatever)/products/page.tsx  (or wherever your ProductsPage lives)
'use client'
...
import ProductEnquiryModal from '@/components/ProductEnquiryModal'
import ProductDetailModal from '@/components/ProductDetailModal'
import { urlFor } from '@/lib/mainclient' // <- use your exported urlFor
...
export default function ProductsPage() {
  ...
  // helper to build URL from a Sanity image object/ref or fallback
  const buildImageUrl = (img: any) => {
    try {
      if (!img) return '/api/placeholder/300/200'

      // If it's already a plain string URL
      if (typeof img === 'string') return img

      // If the server returned object like { asset: { _ref: 'image-...' } } or { asset: { _id: '...' } }
      if (img.asset) {
        // urlFor accepts the image object (most robust). Use width & auto format for performance
        return urlFor(img).width(800).auto('format').url()
      }

      // If the server returned something like { _ref: 'image-...' } directly
      if (img._ref || img.asset?._ref) {
        return urlFor(img).width(800).auto('format').url()
      }
      ...
      console.warn('Error building image url', err, img)
      return '/api/placeholder/300/200'
    }
  }
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      setLoading(true)
      const candidateUrls = ['/api/admin/products']
      ...
      if (fetched.length) {
        const normalized = fetched.map((p: any, idx: number) => {
          ...
          // Build image url from Sanity image object/ref
          let imageUrl = undefined
          if (Array.isArray(p.images) && p.images.length) {
            const first = p.images[0]
            imageUrl = buildImageUrl(first)
          } else if (p.image) {
          ...
          return {
            ...
            brand: brandName,
            specifications,
            // include raw product so detail modal can show original fields if needed
            _raw: p
          }
        })
